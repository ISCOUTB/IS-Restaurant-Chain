...
class TestInventoryIntegration(unittest.TestCase):
    ...
    def test_add_and_get_inventory(self):
        inventory = Inventory(
            product_id=1, name="Product A", stock=100, price=9.99, description="Description of Product A"
        )
    ...
    def test_update_inventory(self):
        ...
        updated_inventory = InventoryUpdate(
            product_id=1, name="Product A Updated", stock=150, price=19.99, description="Updated Description"
        )
    ...
    def test_add_inventory_with_negative_stock(self):
        with self.assertRaises(ValidationError):
            inventory = Inventory(
                product_id=1, name="Product A", stock=-10, price=9.99, description="Description of Product A"
            )
            self.inventory_service.add_inventory(inventory)
    ...
    def test_add_inventory_with_empty_name(self):
        with self.assertRaises(ValidationError):
            inventory = Inventory(
                product_id=1, name="", stock=100, price=9.99, description="Description of Product A"
            )
            self.inventory_service.add_inventory(inventory)

    def test_add_inventory_with_empty_price(self):
        with self.assertRaises(ValidationError):
            inventory = Inventory(
                product_id=1, name="Product A", stock=100, price=0, description="Description of Product A"
            )
            self.inventory_service.add_inventory(inventory)
    ...
    def test_add_and_get_inventory_multiple(self):
        ...
        inventory2 = Inventory(
            product_id=2, name="Product B", stock=200, price=19.99, description="Description of Product B"
        )
    ...
    def test_add_inventory_existing(self):
        inventory = Inventory(
            product_id=1, name="Product A", stock=100, price=9.99, description="Description of Product A"
        )
        self.inventory_service.add_inventory(inventory)
